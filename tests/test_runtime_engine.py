...
async def runtime_engine():
    ...
    config = RuntimeConfig(
        backend=RuntimeBackend.CPU, enable_monitoring=False, enable_caching=False
    )
...
class TestRuntimeEngine:
    ...
    async def test_submit_task(self, runtime_engine):
        ...
        task_id = await runtime_engine.submit_task(
            TaskType.INFERENCE, {"inputs": [1, 2, 3]}, RuntimeBackend.CPU
        )
    ...
    async def test_cpu_inference(self, runtime_engine):
        ...
        task_id = await runtime_engine.submit_task(
            TaskType.INFERENCE, {"inputs": [1, 2, 3, 4, 5], "operation": "mean"}, RuntimeBackend.CPU
        )
    ...
    async def test_cpu_embedding(self, runtime_engine):
        ...
        task_id = await runtime_engine.submit_task(
            TaskType.EMBEDDING,
            {"texts": "Test text for embedding", "dimensions": 128},
            RuntimeBackend.CPU,
        )
    ...
    async def test_backend_selection(self, runtime_engine):
        ...
        backend = runtime_engine._select_backend(TaskType.INFERENCE, RuntimeBackend.CPU)
        assert backend is not None

        # Test auto-selection
        backend = runtime_engine._select_backend(TaskType.INFERENCE, None)
        assert backend is not None
    ...
    async def test_metrics_collection(self, runtime_engine):
        ...
        assert runtime_engine.metrics["tasks_completed"] >= initial_metrics.get(
            "tasks_completed", 0
        )
    ...
    async def test_concurrent_tasks(self, runtime_engine):
        ...
        for i in range(5):
            task_id = await runtime_engine.submit_task(
                TaskType.INFERENCE, {"inputs": [i, i + 1, i + 2]}, RuntimeBackend.CPU
            )
...
class TestBackends:
    ...
    async def test_cpu_backend_operations(self):
        ...
        # Test inference
        result = await backend.inference({"inputs": [1, 2, 3, 4, 5], "operation": "mean"})
        assert result["output"] == 3.0

        # Test embedding
        result = await backend.embed({"texts": ["test"], "dimensions": 64})
    ...
    async def test_onnx_backend_providers(self):
        ...
        from openruntime.backends.onnx_backend import ONNX_AVAILABLE, ONNXBackend
    ...
    async def test_llm_cli_backend(self, mock_run):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout="llm version 0.26.0", stderr="")
