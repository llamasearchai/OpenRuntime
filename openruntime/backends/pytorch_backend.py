    ...
    async def embed(self, payload: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            for text in texts:
                ...
                text_tensor = torch.tensor([float(b) for b in text_bytes], dtype=torch.float32).to(
                    self.device
                )
                ...
                with torch.no_grad():
                    # Layer normalization
                    embedding = torch.nn.functional.layer_norm(text_tensor, text_tensor.shape)
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        if TORCH_AVAILABLE:
            if torch.cuda.is_available():
                ...
                metrics["mps"] = {"available": True, "built": torch.backends.mps.is_built()}
