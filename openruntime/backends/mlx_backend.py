...
class MLXBackend(BaseBackend):
    ...
    async def _load_models(self) -> None:
        ...
        supported_models = ["mistral-7b", "llama-2-7b", "phi-2", "stablelm-2-1.6b", "gemma-2b"]
    ...
    async def _load_model(self, name: str, path: Path) -> None:
        ...
        self.models[name] = {"path": path, "loaded": False}
    ...
    async def inference(self, payload: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            if isinstance(inputs, (list, np.ndarray)):
                ...
                inputs = {
                    k: mx.array(v) if isinstance(v, (list, np.ndarray)) else v
    ...
    async def _complete_via_llm_mlx(
        ...
        try:
            ...
            if result.returncode == 0:
                return {"content": result.stdout, "model": model, "backend": "MLX (via llm-mlx)"}
            else:
